...
def get_sensor_data_older(req: Request, sensor_type: str, limit_num: int):
    ...
    if sensor_type == sensor.THERMOMETER:
        ...
        for r in models.FeverThermometer.objects.filter(account=user)[:int(limit_num)]:
            result_list.append(get_sensor_json_data(sensor_data=r))
        result = result_code.SUCCESS
    elif sensor_type == sensor.PULSE_TRANSDUCER:
        for r in models.PulseTransducer.objects.filter(account=user)[:int(limit_num)]:
            result_list.append(get_sensor_json_data(sensor_data=r))
        result = result_code.SUCCESS
    elif sensor_type == sensor.TURGOSCOPE:
        for r in models.Turgoscope.objects.filter(account=user)[:int(limit_num)]:
            result_list.append(get_sensor_json_data(sensor_data=r))
        result = result_code.SUCCESS
    elif sensor_type == sensor.GPS:
        for r in models.GPS.objects.filter(account=user)[:int(limit_num)]:
            result_list.append(get_sensor_json_data(sensor_data=r))
        result = result_code.SUCCESS
...
def get_sensor_data_older_by_name(req: Request, sensor_type: str, name: str, limit_num: int):
    ...
    result = result_code.MOBILE_SENSOR_TYPE_UNSUPPORTED
    if sensor_type == sensor.THERMOMETER:
        for r in models.Thermometer.objects.filter(account=user, name=name):
            result_list.append(get_sensor_json_data(r))
        result = result_code.SUCCESS
    elif sensor_type == sensor.HUMIDOMETER:
        for r in models.Humidometer.objects.filter(account=user, name=name):
            result_list.append(get_sensor_json_data(r))
        result = result_code.SUCCESS
    elif sensor_type == sensor.SMOKE_TRANSDUCER:
        for r in models.Humidometer.objects.filter(account=user, name=name):
