...
def add_friends_req(req: Request):
    ...
    return view_utils.get_json_response(message=_('Adding friends request has sent'), user_id=user.user_id, search_id=search_id)
...
def add_friends_resp(req: Request):
    ...
    try:
        ...
        raise CheckException(result_code=result_code.MOBILE_USER_INFO_NOT_FOUND, message=_('User does not exist'),
                             status=status.HTTP_404_NOT_FOUND)
    cache_token = cache.get(adding_id + user.user_id)
    if adding_tag == 'accept':
        if cache_token is not None:
            friendship = Friendship(user, adding_user)
            friendship.save()
            publish_message.push_adding_friends_resp_message(adding_tag, user.user_id, adding_id)
            return view_utils.get_json_response(message=_('The friendship has been built successfully'), user_id=user.user_id, adding_id=adding_id)
        else:
        ...
        if cache_token is not None:
            publish_message.push_adding_friends_resp_message(adding_tag, user.user_id, adding_id)
            return view_utils.get_json_response(message=_('You have refused the adding friends request'), user_id=user.user_id, adding_id=adding_id)
        else:
            raise CheckException(result_code=result_code.MOBILE_ADD_FRIENDS_REQUEST_INVALID, message=_('Adding friends request invalid'),
                                 status=status.HTTP_408_REQUEST_TIMEOUT)
    else:
